...
describe('BudgetController.getAll' , () =>{
    ...
    beforeEach(() => {
        ...
        (Budget.findAll as jest.Mock).mockImplementation((options) =>{
            const updatetBudgets = budgets.filter(budget => budget.userId === options.where.userId);
            return Promise.resolve(updatetBudgets)
        })
    })
...
describe('BudgetControlle.updateEditById' ,() =>{
    it('should update the budget and return a success message' , async () =>{
        ...
        const req= createRequest({
            ...
            url: '/api/budgets/:budgetId',
            budget: budgetMock,
            body: {name: 'Presupuesto actualizado correctamente' , amount:5000}
        })
        const res = createResponse();
        await BudgetController.updateEditById(req,res)
...
describe('BudgetControlle.deleteBudgetById' ,() =>{
